...
class SimplePDFParser extends _events["default"] {
  /**
   *
  ...
  constructor(fileBuffer, options = {}) {
    ...
    this.options = _objectSpread({}, DEFAULT_PARSER_OPTIONS, {}, options);
  }
  /**
  ...
  async parse() {
    ...
    const elementsSorted = elements.slice().sort((a, b) => a.pageIndex - b.pageIndex !== 0 ? a.pageIndex - b.pageIndex : a.y - b.y);
    ...
    for (let i = 0; i < elementsSorted.length; i += 1) {
      ...
      const {
        y: _
            ...
            elemClean = (0, _objectWithoutProperties2["default"])(elem, ["y"]);
      ...
      if (deltaY !== null && deltaY > this.options.paragraphThreshold) {
        ...
        if (this.options.joinParagraphs && lastJoined && lastJoined.type === 'text') {
          if (lastJoined.items.length > 0) {
            ...
            lastItem.text = "".concat(lastItem.text, " ");
          }

          lastJoined.items = [...lastJoined.items, ...elemClean.items];
        } else {
          elementsJoined.push(elemClean);
        }
      }
  ...
  async parseRaw() {
    const loadingTask = _pdf["default"].getDocument(new Uint8Array(this.fileBuffer));
    ...
    this.emit('done');
    return results.sort((a, b) => a.index - b.index).map(page => ({
  ...
  async extractPageImages(page) {
    ...
    for (let i = 0; i < ops.fnArray.length; i += 1) {
      ...
      switch (fn) {
        case _pdf["default"].OPS.transform:
          {
            lastMatrix = args;
            break;
          }

        case _pdf["default"].OPS.paintImageXObject:
          {
            ...
            const heightAbs = height > 0 ? height : -height; // x is always relative to the left edge

            const x = lastMatrix[4]; // y might be relative to the top or bottom edge depending on previous
            // transformations. We later calculate `top` later to unify it.
            // y is always relative to the bottom edge of the image.

            const y = lastMatrix[5]; // If height is negative, y is relative to the top edge.
            // If height is positive, y is relative to the bottom edge.
            // Subtract the absolute height.

            const top = (height < 0 ? y : pageHeight - y) - heightAbs; // Clamp the width and height so that they don't overflow the page.
    ...
    if (imageRects.length > 0) {
      ...
      const pageSharp = (0, _sharp["default"])(pageImageBuffer);
      images = await Promise.all(imageRects.map(async elem => _objectSpread({}, elem, {
        imageBuffer: await pageSharp.clone().extract({
        ...
        }).toFormat(this.options.imageOutputFormat).toBuffer()
      })));
    }
...
exports["default"] = SimplePDFParser;